...
def run_migrations(sql_dir: str, db_url: str):
    ...
    sql_files = sorted(f for f in os.listdir(sql_dir) if f.lower().endswith(".sql"))
    if not sql_files:
...
def run_env_sql(db_url: str):
    ...
    try:
        ...
        cursor.execute(insert_sql, (api_key, api_key, project_id))
        conn.commit()
        cursor.close()
        conn.close()
...
def main():
    if os.getenv("RUN_MIGRATION", "0") == "1":
        ...
        run_migrations(
            os.getenv("SQL_DIR", "."),
            os.getenv("DB_URL", "postgresql://user:pass@localhost:5432/mydb"),
        )
